...
class AutoCommentArticleJob : Job {
    ...
    override fun execute(context: JobExecutionContext) {
        Settings.community.watchTags.forEach { tag ->
            ArticleCall.getArticlesByTag(tag).data?.articles?.forEach { article ->
                ArticleCall.getArticle(article.oId).data?.article?.let { ad ->
                    ...
                    CommentCall.comment(article.oId, Settings.community.commentTexts.random())
